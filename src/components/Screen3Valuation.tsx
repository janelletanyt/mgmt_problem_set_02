...
export const Screen3Valuation: React.FC<Screen3ValuationProps> = ({
  ...
  return (
    <div className="p-4 sm:p-5 space-y-5">
      ...
      <div className="bg-slate-100/90 rounded-xl p-3 border border-slate-200/80 flex items-center justify-between text-xs">
        <div className="space-y-0.5 truncate pr-2">
          <div className="flex items-center space-x-1 font-semibold text-slate-900 truncate">
            ...
            <span className="truncate">{valuation.address}, {valuation.unitNumber}</span>
          </div>
          <div className="flex items-center space-x-1 text-slate-500 text-[11px]">
            <Calendar className="w-3 h-3 text-slate-400" />
            <span>Horizon: +{valuation.targetHorizonYears}y (Year {valuation.targetCalendarYear})</span>
          </div>
        </div>
      ...
      {/* =========================================================================
          THE 4 API STATE NOTICES (Data Loading handled in App.tsx / below for empty, refused, unreachable)
         ========================================================================= */}

      {/* State 2: THE DATA IS EMPTY */}
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-900 space-y-2">
          <div className="flex items-start space-x-2.5">
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-amber-950 text-sm">
                No historical resale transactions were found for this estate on data.gov.sg.
              </h4>
              <p className="text-[11px] text-amber-800 leading-relaxed">
                The upstream dataset returned zero matching records for this query. The figures below reflect the baseline statistical model while you adjust your address.
              </p>
              {valuation.statusDetail && (
                <p className="text-[10px] text-amber-700 font-mono bg-amber-100/80 px-2 py-1 rounded">
                  {valuation.statusDetail}
                </p>
              )}
            </div>
          ...
          <div className="pt-1 flex gap-2">
            <button
              ...
              className="text-xs bg-amber-700 hover:bg-amber-800 text-white font-medium px-3 py-1.5 rounded-lg transition-colors"
            >
              Modify Estate Address
            </button>
          </div>
        </div>
      )}
      ...
      {valuation.apiStatus === 'refused' && (
        <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 text-xs text-rose-950 space-y-2.5">
          <div className="flex items-start space-x-2.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-rose-800 leading-relaxed">
                The serverless function halted before or upon calling the upstream endpoint.
              </p>
              {valuation.statusDetail && (
          ...
          <div className="text-[11px] text-rose-700 bg-white/80 p-2.5 rounded-lg border border-rose-200/80">
            <strong>Note for Vercel deployment:</strong> Set the environment variable <code className="bg-rose-100 px-1 py-0.5 rounded font-mono text-[10px]">DATA_GOV_SG_API_KEY</code> in Vercel Project Settings &gt; Environment Variables.
          </div>
          {onRetry && (
            <button
              onClick={onRetry}
      ...
      {valuation.apiStatus === 'unreachable' && (
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-xs text-orange-950 space-y-2">
          <div className="flex items-start space-x-2.5">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-orange-800 leading-relaxed">
                Could not connect to data.gov.sg API. Showing local baseline projection.
              </p>
              {valuation.statusDetail && (
          ...
          {onRetry && (
            <button
              onClick={onRetry}
              className="text-xs bg-orange-600 hover:bg-orange-700 text-white font-medium px-3 py-1.5 rounded-lg transition-colors"
            >
              Retry Connection
            </button>
          )}
      ...
      {/* LIVE DATA SUCCESS BADGE */}
      {valuation.apiStatus === 'success' && (
        ...
        <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-100 text-left">
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
            <span className="text-[10px] uppercase font-semibold text-slate-400 block">
              {valuation.apiStatus === 'success' ? 'Live Estate Median' : 'Baseline Reference'}
            </span>
          ...
          <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
            <span className="text-[10px] uppercase font-semibold text-slate-400 block">
              {valuation.apiStatus === 'success' ? 'Live Historical CAGR' : 'Projected Growth'}
            </span>
